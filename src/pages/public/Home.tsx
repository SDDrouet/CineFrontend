...
const CLASSIFICATIONS = Array.from(new Set(movies.map((m) => m.classification)));
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      <section className="mb-10 rounded-2xl bg-gradient-to-br from-zinc-900 via-zinc-900 to-amber-950 px-8 py-14 text-white">
        ...
        <h1 className="max-w-2xl text-4xl font-bold leading-tight sm:text-5xl">
          Reserva tu butaca antes de que se apaguen las luces
        </h1>
        <p className="mt-4 max-w-xl text-zinc-300">
          Elige película, función y asiento en pocos pasos. VIP, DBOX o General: tú decides cómo ver la historia.
        </p>
      </section>

      <section className="mb-8 grid grid-cols-1 gap-3 rounded-xl border p-4 sm:grid-cols-2 lg:grid-cols-5">
        <Input placeholder="Buscar por título" value={name} onChange={(e) => setName(e.target.value)} />

        <select className="rounded-md border px-3 py-2 text-sm" value={genre} onChange={(e) => setGenre(e.target.value)}>
      ...
      {filteredMovies.length === 0 ? (
        <p className="py-16 text-center text-zinc-500">
          No encontramos películas con esos filtros. Prueba con otra combinación.
        </p>
      ) : (
